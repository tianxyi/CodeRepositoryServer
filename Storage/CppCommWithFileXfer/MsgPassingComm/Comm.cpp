...
using SUtils = Utilities1::StringHelper;
//constant not can changeable  dont runtime but could compile time
const std::string saveFilePath = "../../SaveFiles";//get store here meiyong?
const std::string sendFilePath = "../convertedPages";//send find name here
...
{
  listener.start(co);//qqqqqqqqqqqqqqqqqqqqqqqqq
}
...
void Sender::start()
{		// Callable Object ,Client handler,how to deal with the msg
  std::function <void()> threadProc = [&]() {//lambda start scope
    while (true)
    {
      ...
      if (msg.command() == "quit")//normal way to close thread
      {
        StaticLogger<1>::write("\n  -- send thread shutting down");
        return;
      }
      StaticLogger<1>::write("\n  -- " + sndrName + " send thread sending " + msg.name());
      std::string msgStr = msg.toString();//serialize

      if (msg.to().address != lastEP.address || msg.to().port != lastEP.port)//where is going to
      {
        connecter.shutDown();
        //connecter.close();
        StaticLogger<1>::write("\n  -- attempting to connect to new endpoint: " + msg.to().toString());
        if (!connect(msg.to()))//create new connection
        {
          StaticLogger<1>::write("\n can't connect");
          continue;//throw msg away got new msg ,not reacheable send another
        }
        else
        {
	  ...
	  //file or normal msg..
      if (msg.containsKey("file"))//19.file!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
      {
        sendFile(msg);
      }
      else
      {
        bool sendRslt = connecter.send(msgStr.length(), (Socket::byte*)msgStr.c_str());//19.noramlmessage
      }
    }
											};//lambda end scope
...
bool Sender::sendFile(Message msg)
{
  if (!msg.containsKey("file"))
    return false;
  std::string fileSpec = sendFilePath + "/" + msg.file();//19.
...
public:
  ...
  std::string readMsg(Socket& socket)
  {
    ...
    {
      temp = socket.recvString('\n');  // read attribute from socket  and frame msg
      msgString += temp;
  ...
  */
  bool receiveFile(Message msg)
  {
    ...
    while (true)
    {
      ...
      msg = Message::fromString(msgString);//deserilazation
      if (msg.contentLength() == 0)
        break;
    }
    saveStream.flush();
    saveStream.close();
    pQ_->enQ(msg);// put msg in the recvque #1
    return true;
  }

  //----< reads messages from socket and enQs in rcvQ >--------------

  void operator()(Socket socket)//Callable Obejcet,argument= Socket
  {		//CO(Socket)       will be called on client socket
     pSocket = &socket;
    while (socket.validState())
    {
      std::string msgString = readMsg(socket);//read msg a s string
      if (msgString.length() == 0)
      {
        // invalid message
        break;
      }
      Message msg = Message::fromString(msgString);//creating msg class from string
													//serilization,deserilization
      StaticLogger<1>::write("\n  -- " + clientHandlerName + " RecvThread read message: " + msg.name());
      //std::cout << "\n  -- " + clientHandlerName + " RecvThread read message: " + msg.name();

      if (msg.containsKey("file"))//msg send file//??????????????
      {
        receiveFile(msg);
      }

      pQ_->enQ(msg);//receive queue go to receiver  #2
...
void Comm::start()
{
  BlockingQueue<Message>* pQ = rcvr.queue();//bq owened by receiver
...
void DemoSndrRcvr(const std::string& machineName)
{
  ...
  msg.name("msg #2");
  msg.to(EndPoint(machineName, 9092));//y machinename,9092
  StaticLogger<1>::flush();
  std::cout << "\n  sndr in main posting message:  " << msg.name();
  sndr.postMessage(msg);
  ...
  msg.name("msg #4");
  msg.to(EndPoint("localhost", 9091));//y
  StaticLogger<1>::flush();
  std::cout << "\n  sndr in main posting message:  " << msg.name();
...
void ThreadProcClnt1()
{
  ...
  {
    Message msg(serverEP, clientEP);//(to,from)
  ...
  Message fileMsg(serverEP, clientEP);//to,from
...
{
  Comm comm(EndPoint("localhost", 9892), "client2Comm");//endpoint,string name
  comm.start();
  EndPoint serverEP("localhost", 9890);
...
void DemoClientServer()
{
  ...
  while (true)
  {
    ...
    msg = comm.getMessage();//qqqqqqqqqqqqqqqqqqqqqq
    std::cout << "\n  " + comm.name() + " received message: " << msg.name();

    if (msg.containsKey("file"))  // is this a file or message?file
    {
      if(msg.contentLength() == 0)
    ...
    else  // non-file message
    {
      rply.to(msg.from());
      rply.from(serverEP);
      rply.name("server reply #" + Utilities::Converter<size_t>::toString(++count) + " to " + msg.from().toString());//name is message?

      comm.postMessage(rply);//qqqqqqqqqqqqqqqqqqqqqq
      if (msg.command() == "stop")
      {
        break;
      }
