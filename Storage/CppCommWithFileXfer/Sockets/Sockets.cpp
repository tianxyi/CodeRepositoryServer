...
{
  int iResult = WSAStartup(MAKEWORD(2, 2), &wsaData);//WSAStartup
  if (iResult != 0) {
...
bool Socket::send(size_t bytes, byte* buffer)
{
  ...
  while (bytesLeft > 0)
  {
    bytesSent = ::send(socket_, pBuf, bytesLeft, 0);//how many send buffer byte
...
{
  size_t uport = htons((u_short)port);//converts short from host to network
...
//-----------------------------------------------------
//an example Callable Object, ClientProc
//below is the Callable Oject,Functor that professor design to handle the msg in childthread,Ckient handler

class ClientHandler
{
public:
  void operator()(Socket& socket_);//operator overloading
