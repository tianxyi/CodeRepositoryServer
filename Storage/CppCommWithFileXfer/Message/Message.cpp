...
Message::Keys Message::keys()
{
  ...
  {
    keys.push_back(kv.first);//assemble of Attribute not value
  }
  return keys;
}
...
size_t Message::contentLength()
{
  if (containsKey("content-length"))
  {
    ...
    return Utilities1::Converter<size_t>::toValue(lenStr);
  }
  return 0;
}
//----< set body length >----------------------------------------------
...
{
  attributes_["content-length"] = Utilities1::Converter<size_t>::toString(ln);
}
//std::string MsgPassingCommunication::Message::Regex()
//{
//  if (containsKey("Regex"))
//  {
//    return attributes_["Regex"];
//  }
//  return "";
//}
...
{
  Message msg;
  std::vector<std::string> splits = Utilities1::StringHelper::split(src);
